...
const Start: React.FC = () => {
  ...
  const {star, addStar} = React.useContext(StarContext)
  ...
  return (
    <div className="rating">
      <div>
      {
        starLength.map((index) => <AiFillStar key={index} onClick={() => addStar && addStar(index)} color={index <= star ? stared : unstar} size="32" />)
      }
      </div>
    </div>
  );
